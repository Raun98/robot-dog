...
[ESP32-S3 N16R8] --I2C (level shift)--> [PCA9685] --> 12x MG995
       |-- I2C --> [BNO055]
       |-- analog --> [ACS712 30A on 6V rail]
       |-- GPIO --> e-stop sense
6V stall-rated BEC --> MG995 V+ (e-stop cuts this rail)
5V 5A class --> Pi only (not servos)
```
...
| Power | Separate Pi 5V vs servo **6 V, stall-rated (~20–30 A class)** | **Yes** if the bench PSU cannot hold twelve MG995s |
